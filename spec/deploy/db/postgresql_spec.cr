...
describe Deploy::DB::Factory do
  it "instancie DB::Postgresql pour database=postgresql" do
    ...
    db = Deploy::DB::Factory.for("postgresql", config, env)
    db.should be_a(Deploy::DB::Postgresql)
  end

  it "instancie DB::None pour database=none" do
    config = SpecHelper.marten_config
    env = SpecHelper.dev_env(config)
    db = Deploy::DB::Factory.for("none", config, env)
    db.should be_a(Deploy::DB::None)
  end

  it "instancie DB::None pour database vide" do
    config = SpecHelper.marten_config
    env = SpecHelper.dev_env(config)
    db = Deploy::DB::Factory.for("", config, env)
    db.should be_a(Deploy::DB::None)
  end
  ...
  it "DB::None#run_dialog retourne un hash vide" do
    config = SpecHelper.marten_config
    env = SpecHelper.dev_env(config)
    db = Deploy::DB::None.new(config, env)
...
describe Deploy::DB::Postgresql, "choix par défaut" do
  ...
  it "le prompt de choix affiche [1] comme valeur par défaut" do
    ...
    config = SpecHelper.marten_config
    env = SpecHelper.dev_env(config)
    db = Deploy::DB::Postgresql.new(config, env)
...
describe Deploy::DB::Postgresql, "non-régression" do
