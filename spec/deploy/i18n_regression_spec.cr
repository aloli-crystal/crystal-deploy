...
describe Deploy::I18n, "non-régression DB_POOL_SIZE" do
  before_each do
    Deploy::I18n.reset
  ...
  {% for lang in ["fr", "en", "es", "de", "pt", "it", "nl"] %}
    it "traduit db.pool_size_prompt en {{ lang.id }}" do
      ...
      result = Deploy::I18n.t("db.pool_size_prompt")
...
describe Deploy::Config, "#effective_env_vars non-régression" do
