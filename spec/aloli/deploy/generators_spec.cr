...
describe Aloli::Deploy::Generators::Nginx do
  it "génère un nginx.conf avec le bon server_name (sans https://)" do
    ...
    content.should contain("server_name dev.mon-app.aloli.app")
...
describe Aloli::Deploy::Generators::Rcd do
  it "génère un script rc.d avec le bon nom de service" do
    config, env = sample_config_and_env
    gen = Aloli::Deploy::Generators::Rcd.new(config, env)
    content = gen.generate
