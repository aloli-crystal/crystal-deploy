...
describe CrystalDeploy::EnvParser do
  it "parse les paires clé=valeur simples" do
    content = "KEY=value\nDB_HOST=localhost"
    result = CrystalDeploy::EnvParser.parse(content)
  ...
  it "retire les guillemets doubles englobants" do
    result = CrystalDeploy::EnvParser.parse(%(KEY="value with spaces"))
  ...
  it "préserve les = dans les valeurs (URLs)" do
    result = CrystalDeploy::EnvParser.parse("DATABASE_URL=postgres://user:pass@host/db?opt=1")
  ...
  it "génère des exports correctement échappés" do
    ...
    exports = CrystalDeploy::EnvParser.generate_exports(content)
  ...
  it "échappe les guillemets doubles dans les valeurs" do
    exports = CrystalDeploy::EnvParser.generate_exports("KEY=val\"ue")
